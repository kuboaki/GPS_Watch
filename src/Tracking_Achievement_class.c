...
{
  Tracking_Goal * goal=0;Tracking_WorkoutTimer * workoutTimer=0;
  /* SELECT one goal RELATED BY self->Goal[R14.is_open_for] */
  XTUML_OAL_STMT_TRACE( 1, "SELECT one goal RELATED BY self->Goal[R14.is_open_for]" );
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT one workoutTimer RELATED BY goal->WorkoutSession[R11.is_currently_executing_within]->WorkoutTimer[R8.is_timed_by]" );
  ...
  ((Tracking_Achievement *)xtUML_detect_empty_handle( self, "Achievement", "self.endTime" ))->endTime = ((Tracking_WorkoutTimer *)xtUML_detect_empty_handle( workoutTimer, "WorkoutTimer", "workoutTimer.time" ))->time;
...
Tracking_Achievement_R12_Link_has_recorded( Tracking_Goal * part, Tracking_Achievement * form )
{
  ...
  form->goal_ID = part->ID;
  form->spec_sequenceNumber = part->spec_sequenceNumber;
