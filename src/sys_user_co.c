...
/* Activate this invocation to initialize the example simple TIM.  */
  ...
  if (!bcm2835_init())
    {
      puts("Unable to initialize BCM library");
      exit(1);
    }

  // t = tm1638_alloc(17, 21, 22);
  gT = tm1638_alloc(19, 13, 06);
  if (!gT)
    {
    puts("Unable to allocate TM1638");
    exit(1);
    }
...
UserBackgroundProcessingCalloutf( void )
{
  ...
  gButtons = tm1638_read_8buttons(gT);
  if( gButtons != buttons_saved ) {
    switch(gButtons) {
    case 1 << 4:
    ...
    case 4 << 4:
      ...
      break;
    default:
      gLeds &= 0x0f;    /* off, leftside 4 leds */
      break;
    }
    buttons_saved = gButtons;
    tm1638_set_8leds(gT, gLeds, 0);    
  }
}
