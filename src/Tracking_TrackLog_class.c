...
{
  bool isFirstTrackPoint;r_t lastLatitude;r_t lastLongitude;r_t distance;Tracking_TrackPoint * trackPoint;Tracking_WorkoutTimer * workoutTimer=0;Tracking_TrackPoint * firstPoint=0;Tracking_TrackPoint * lastPoint=0;Tracking_WorkoutSession * session=0;
  /* SELECT one workoutTimer RELATED BY self->WorkoutSession[R4.represents_path_for]->WorkoutTimer[R8.is_timed_by] */
  XTUML_OAL_STMT_TRACE( 1, "SELECT one workoutTimer RELATED BY self->WorkoutSession[R4.represents_path_for]->WorkoutTimer[R8.is_timed_by]" );
  ...
  ((Tracking_TrackPoint *)xtUML_detect_empty_handle( trackPoint, "TrackPoint", "trackPoint.time" ))->time = ((Tracking_WorkoutTimer *)xtUML_detect_empty_handle( workoutTimer, "WorkoutTimer", "workoutTimer.time" ))->time;
  ...
  Tracking_LOC_getLocation( &(((Tracking_TrackPoint *)xtUML_detect_empty_handle( trackPoint, "TrackPoint", "trackPoint.latitude" ))->latitude), &(((Tracking_TrackPoint *)xtUML_detect_empty_handle( trackPoint, "TrackPoint", "trackPoint.longitude" ))->longitude) );
  ...
  if ( ( 0 == firstPoint ) ) {
    ...
    Tracking_TrackPoint_R3_Link_has_last( self, trackPoint );
  }
  else {
    /* ASSIGN lastLatitude = lastPoint.latitude */
    XTUML_OAL_STMT_TRACE( 2, "ASSIGN lastLatitude = lastPoint.latitude" );
    lastLatitude = ((Tracking_TrackPoint *)xtUML_detect_empty_handle( lastPoint, "TrackPoint", "lastPoint.latitude" ))->latitude;
    /* ASSIGN lastLongitude = lastPoint.longitude */
    XTUML_OAL_STMT_TRACE( 2, "ASSIGN lastLongitude = lastPoint.longitude" );
    lastLongitude = ((Tracking_TrackPoint *)xtUML_detect_empty_handle( lastPoint, "TrackPoint", "lastPoint.longitude" ))->longitude;
    /* UNRELATE self FROM lastPoint ACROSS R3 */
    XTUML_OAL_STMT_TRACE( 2, "UNRELATE self FROM lastPoint ACROSS R3" );
    Tracking_TrackPoint_R3_Unlink_has_last( self, lastPoint );
    /* RELATE self TO trackPoint ACROSS R3 */
    XTUML_OAL_STMT_TRACE( 2, "RELATE self TO trackPoint ACROSS R3" );
    Tracking_TrackPoint_R3_Link_has_last( self, trackPoint );
  ...
  if ( !isFirstTrackPoint ) {
    /* LOC::getDistance(fromLat:lastLatitude, fromLong:lastLongitude, result:distance, toLat:trackPoint.latitude, toLong:trackPoint.longitude) */
    XTUML_OAL_STMT_TRACE( 2, "LOC::getDistance(fromLat:lastLatitude, fromLong:lastLongitude, result:distance, toLat:trackPoint.latitude, toLong:trackPoint.longitude)" );
    Tracking_LOC_getDistance( lastLatitude, lastLongitude, &(distance), ((Tracking_TrackPoint *)xtUML_detect_empty_handle( trackPoint, "TrackPoint", "trackPoint.latitude" ))->latitude, ((Tracking_TrackPoint *)xtUML_detect_empty_handle( trackPoint, "TrackPoint", "trackPoint.longitude" ))->longitude );
  }
  /* SELECT one session RELATED BY self->WorkoutSession[R4.represents_path_for] */
  XTUML_OAL_STMT_TRACE( 1, "SELECT one session RELATED BY self->WorkoutSession[R4.represents_path_for]" );
  ...
  ((Tracking_WorkoutSession *)xtUML_detect_empty_handle( session, "WorkoutSession", "session.accumulatedDistance" ))->accumulatedDistance = ( ((Tracking_WorkoutSession *)xtUML_detect_empty_handle( session, "WorkoutSession", "session.accumulatedDistance" ))->accumulatedDistance + distance );
  /* self.updateDisplay() */
...
{
  Tracking_TrackPoint * nextPoint=0;Tracking_TrackPoint * lastPoint=0;
...
Tracking_TrackLog_op_addLapMarker( Tracking_TrackLog * self)
{
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT one timer RELATED BY self->WorkoutSession[R4.represents_path_for]->WorkoutTimer[R8.is_timed_by]" );
  ...
  ((Tracking_LapMarker *)xtUML_detect_empty_handle( lapMarker, "LapMarker", "lapMarker.lapTime" ))->lapTime = ((Tracking_WorkoutTimer *)xtUML_detect_empty_handle( timer, "WorkoutTimer", "timer.time" ))->time;
...
Tracking_TrackLog_op_updateDisplay( Tracking_TrackLog * self)
{
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT one display RELATED BY self->WorkoutSession[R4.represents_path_for]->Display[R7.current_status_indicated_on]" );
