...
UI_GuiBridge_setData( const i_t p_unit, const r_t p_value )
{
  ...
  gSeg_buf[4] = '\0';
  strncat(gSeg_buf, buf, 4);
  
  gLeds = p_unit;

  tm1638_set_8leds(gT, gLeds, 0);
  tm1638_set_7seg_text(gT, gSeg_buf, 0x02);
}
...
UI_GuiBridge_setTime( const i_t p_time )
{
  ...
  int min = (p_time % 3600) / 60;
  int sec = p_time % 60;
  sprintf(gSeg_buf, "%02d%02d", min, sec);
