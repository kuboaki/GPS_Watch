...
{
  GPS_Watch_Indicator_t indicator;Tracking_WorkoutSession * session=0;Tracking_Goal * goal=0;
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT one goal RELATED BY session->Goal[R11.is_currently_executing]" );
  ...
  indicator = GPS_Watch_Indicator_Blank_e;
  /* IF ( not empty goal ) */
  XTUML_OAL_STMT_TRACE( 1, "IF ( not empty goal )" );
  if ( !( 0 == goal ) ) {
    /* IF ( ( goal.disposition == Increase ) ) */
    XTUML_OAL_STMT_TRACE( 2, "IF ( ( goal.disposition == Increase ) )" );
    if ( ( ((Tracking_Goal *)xtUML_detect_empty_handle( goal, "Goal", "goal.disposition" ))->disposition == GPS_Watch_GoalDisposition_Increase_e ) ) {
      /* ASSIGN indicator = Up */
      XTUML_OAL_STMT_TRACE( 3, "ASSIGN indicator = Up" );
      indicator = GPS_Watch_Indicator_Up_e;
    }
    else if ( ( ((Tracking_Goal *)xtUML_detect_empty_handle( goal, "Goal", "goal.disposition" ))->disposition == GPS_Watch_GoalDisposition_Decrease_e ) ) {
      /* ASSIGN indicator = Down */
      XTUML_OAL_STMT_TRACE( 3, "ASSIGN indicator = Down" );
      indicator = GPS_Watch_Indicator_Down_e;
    }
    else {
      /* ASSIGN indicator = Flat */
      XTUML_OAL_STMT_TRACE( 3, "ASSIGN indicator = Flat" );
      indicator = GPS_Watch_Indicator_Flat_e;
    }
  }
  /* RETURN indicator */
  XTUML_OAL_STMT_TRACE( 1, "RETURN indicator" );
  {GPS_Watch_Indicator_t xtumlOALrv = indicator;
  return xtumlOALrv;}
}
...
Tracking_Display_act1( Tracking_Display * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT one session RELATED BY self->WorkoutSession[R7.indicates_current_status_of]" );
  ...
  distance = ((Tracking_WorkoutSession *)xtUML_detect_empty_handle( session, "WorkoutSession", "session.accumulatedDistance" ))->accumulatedDistance;
  /* IF ( ( distance > 1000.0 ) ) */
  ...
  if ( ( distance > 1000.0 ) ) {
    ...
    Tracking_UI_setData( GPS_Watch_Unit_km_e, ( distance / 1000.0 ) );
  }
  else {
    /* UI::setData(unit:meters, value:distance) */
    XTUML_OAL_STMT_TRACE( 2, "UI::setData(unit:meters, value:distance)" );
    Tracking_UI_setData( GPS_Watch_Unit_meters_e, distance );
  }
...
Tracking_Display_act2( Tracking_Display * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  session = ( 0 != self ) ? self->WorkoutSession_R7_indicates_current_status_of : 0;
  /* ASSIGN speed = session.getCurrentSpeed() */
  XTUML_OAL_STMT_TRACE( 1, "ASSIGN speed = session.getCurrentSpeed()" );
  speed = Tracking_WorkoutSession_op_getCurrentSpeed(session);
  /* UI::setData(unit:kmPerHour, value:speed) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setData(unit:kmPerHour, value:speed)" );
  Tracking_UI_setData( GPS_Watch_Unit_kmPerHour_e, speed );
  /* UI::setIndicator(Display::goalDispositionIndicator()) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setIndicator(Display::goalDispositionIndicator())" );
  Tracking_UI_setIndicator( Tracking_Display_op_goalDispositionIndicator() );
}
...
Tracking_Display_act3( Tracking_Display * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  session = ( 0 != self ) ? self->WorkoutSession_R7_indicates_current_status_of : 0;
  /* ASSIGN pace = session.getCurrentPace() */
  XTUML_OAL_STMT_TRACE( 1, "ASSIGN pace = session.getCurrentPace()" );
  pace = Tracking_WorkoutSession_op_getCurrentPace(session);
  /* UI::setData(unit:minPerKm, value:pace) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setData(unit:minPerKm, value:pace)" );
  Tracking_UI_setData( GPS_Watch_Unit_minPerKm_e, pace );
  /* UI::setIndicator(Display::goalDispositionIndicator()) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setIndicator(Display::goalDispositionIndicator())" );
  Tracking_UI_setIndicator( Tracking_Display_op_goalDispositionIndicator() );
}
...
Tracking_Display_act4( Tracking_Display * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  session = ( 0 != self ) ? self->WorkoutSession_R7_indicates_current_status_of : 0;
  /* ASSIGN heartRate = session.getCurrentHeartRate() */
  XTUML_OAL_STMT_TRACE( 1, "ASSIGN heartRate = session.getCurrentHeartRate()" );
  heartRate = Tracking_WorkoutSession_op_getCurrentHeartRate(session);
  /* UI::setData(unit:bpm, value:heartRate) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setData(unit:bpm, value:heartRate)" );
  Tracking_UI_setData( GPS_Watch_Unit_bpm_e, heartRate );
  /* UI::setIndicator(Display::goalDispositionIndicator()) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setIndicator(Display::goalDispositionIndicator())" );
  Tracking_UI_setIndicator( Tracking_Display_op_goalDispositionIndicator() );
}
...
Tracking_Display_act5( Tracking_Display * self, const Escher_xtUMLEvent_t * const event )
{
  ...
  XTUML_OAL_STMT_TRACE( 1, "SELECT many lapMarkers RELATED BY self->WorkoutSession[R7.indicates_current_status_of]->TrackLog[R4.captures_path_in]->LapMarker[R5.has_laps_defined_by]" );
  ...
  Tracking_UI_setData( GPS_Watch_Unit_laps_e, Escher_SetCardinality( lapMarkers ) );
  /* UI::setIndicator(Display::goalDispositionIndicator()) */
  XTUML_OAL_STMT_TRACE( 1, "UI::setIndicator(Display::goalDispositionIndicator())" );
  Tracking_UI_setIndicator( Tracking_Display_op_goalDispositionIndicator() );
  Escher_ClearSet( lapMarkers ); 
