...
Raspberry Pi で作る GPS Watch の完成品は、次のような概観になります。
...
UMLでモデルを作成するコアデバイスは、次のようなコンポーネント構造になっています。
...
各コンポーネントはクラスの集まりになっており、それぞれのクラスの振舞いはステートマシン図で表します。
...
さらに、それぞれの状態ごとの処理（それぞれの状態の中に書いてあるスクリプト）は、モデルの振舞いを記述するための「アクション言語（OAL: Object Action Language）」を使って書きます。
...
生成されたソース・コードは 次のような感じになります。暗号のようなコードではなく、手書きのコードに近いことがわかりますね。

.GPS_Watch/src/Tracking_Display_class.h
[source, c, linenums, start=51]
----
include::{sourcesdir}/Tracking_Display_class.h[lines=51..65]
----


.GPS_Watch/src/Tracking_Display_class.c
[source, c, linenums, start=242]
----
include::{sourcesdir}/Tracking_Display_class.c[lines=242..255]
----

これらに TM1638ボードを接続した Raspberry Pi  動作させるための手書きコードを追加したのが、このリポジトリにあるC言語ソース・コードになります。
とはいっても、生成したコードは書き換えることはありません。手書きコード用の関数にTM1638ボード用のAPIを書き足すだけです。
...
image::createing_launch_tab_configration_error.png[createing_launch_tab_configration_error,{three-quarters-width}]
